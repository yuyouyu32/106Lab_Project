...
for file in picfiles:
    ...
    with open(path+'/'+file,'r') as fin:
        ...
        titlename = str(piccnt)+'D Result'
        plt.title(titlename)
        plt.xlabel("Measure Data")
...
# for file_index in picturename:
#     before = file_index
#     after = file_index.split('/')[1]
#     os.system(f'copy {before} {after}')
...
with open('result_before.xml','r') as fin:
    with open('result.xml','w') as fout:
        ...
        for i in range(0,dim):
            fout.write('    <formula>' + formula[i] + '</formula>\n')
            fout.write('    <score>'+score[i]+'</score>\n')
        fout.write('    <picture>\n')
        for i in range(0,dim):
            fout.write('      <file>\n')
